...
export var velocidad_maxima: Vector2 = Vector2(10.0, 60.0)
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("disparar"):
		arbol_animacion.set_valor_mezcla_disparar(1)
		disparando = true
		linterna.light_energy = 15
	elif event.is_action_released("disparar"):
		arbol_animacion.set_valor_mezcla_disparar(0)
		disparando = false
		linterna.light_energy = 0

## Metodos Custom
func movimiento_horizontal() -> void:
	movimiento.x = tomar_direccion().x * velocidad_maxima.x
	movimiento.z = tomar_direccion().z * velocidad_maxima.x
...
func movimiento_vertical() -> void:
	if not is_on_floor():
		...
		movimiento.y = clamp(movimiento.y, -velocidad_maxima.y, impulso)
	...
	var inicio_salto: bool = is_on_floor() and Input.is_action_just_pressed("salto")
	
	if inicio_salto:
		arbol_animacion.set_valor_transicion_suelo_aire(AIRE)
		arbol_animacion.set_valor_mezcla_saltar_caer(0)
		vector_snap = Vector3.ZERO
		saltando = true
		salto_interrumpido = false
		cayendo = false
	elif tocando_suelo:
		arbol_animacion.set_valor_transicion_suelo_aire(SUELO)
		vector_snap = Vector3.DOWN
	
	if movimiento.y >= velocidad_maxima.y:
		salto_interrumpido = true
	
	if Input.is_action_pressed("salto") and saltando and not salto_interrumpido:
